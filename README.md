...
必要ならルートの `.env.example` を `.env` にコピーし、`OPENAI_API_KEY` を設定してください。proposal / materialization proposal / query の生成には OpenAI 互換 API 設定が必要です。
アプリ起動後は左ペインの `LLM Settings` から `API Key` `Endpoint` `Model` を保存でき、以後の proposal / materialization proposal / query に反映されます。
